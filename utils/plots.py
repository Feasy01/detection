...
def plot_one_box(x, im, color=(128, 128, 128), label=None, line_thickness=3, proportion = 0.20):
    # Plots one bounding box on image 'im' using OpenCV
    assert im.data.contiguous, 'Image not contiguous. Apply np.ascontiguousarray(im) to plot_on_box() input image.'
    tl = line_thickness or round(0.002 * (im.shape[0] + im.shape[1]) / 2) + 1  # line/font thickness
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    deltax = int((c2[0] - c1[0]) * float(proportion))
    deltay = int((c2[1] - c1[1]) * float(proportion))
    cv2.line(im, c1, (c1[0] + deltax, c1[1]), color, thickness=tl, )
    cv2.line(im, c1, (c1[0], c1[1] + deltay), color, thickness=tl, )
    cv2.line(im, c2, (c2[0] - deltax, c2[1]), color, thickness=tl, )
    cv2.line(im, c2, (c2[0], c2[1] - deltay), color, thickness=tl,)
    cv2.line(im, (c1[0],c2[1]), (c1[0]+deltax, c2[1]), color, thickness=tl, )
    cv2.line(im, (c1[0], c2[1]), (c1[0], c2[1]-deltay), color, thickness=tl, )
    cv2.line(im, (c2[0],c1[1]), (c2[0] - deltax, c1[1]), color, thickness=tl, )
    cv2.line(im, (c2[0], c1[1]), (c2[0], c1[1]+deltay), color, thickness=tl, )
    if label:
